...
def run_simulation(N, numSeeds, alg):
    ...
    if alg == "ex":
        for i in range(N, 21):
            nString = str(i) + "_" + str(numSeeds) + "ex.txt"
            with open(nString, "a") as f:
                start = time.time()
                f.write(str(i) + "-Dimensional System - Ground States (seed, configuration)")
                f.write("\n")
                for seed in range(0,numSeeds):
                    Jij = gj.generateJij(i, seed=seed)
                    bm = gj.exhaustive(Jij)
